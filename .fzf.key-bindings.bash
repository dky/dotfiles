...
__fzf_alias__() {
  ...
  alias_cmd=$(eval "$cmd" | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS" $(__fzfcmd) --preview "grep {} ~/.dotfiles/.custom_aliases | sed -e 's/^alias //g' | cut -d= -f 2" +m --preview-window=top:10%) && printf '%q' "$alias_cmd"
}
