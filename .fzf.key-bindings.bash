...
__fzf_cd__() {
  local cmd dir
  cmd="${FZF_ALT_C_COMMAND:-"command find -L . -mindepth 1 \
    \\( -path '*/\\.*' -o -fstype 'sysfs' -o -fstype 'devfs' -o -fstype 'devtmpfs' -o -fstype 'proc' \\) -prune \
...
__fzf_history__() {
  ...
  output=$(
    builtin fc -lnr -2147483648 |
      ...
      FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-60%} $FZF_DEFAULT_OPTS -n2..,.. --tiebreak=index --bind=ctrl-r:toggle-sort $FZF_CTRL_R_OPTS +m --read0" $(__fzfcmd) --query "$READLINE_LINE"
  ) || return
...
__fzf_alias__() {
  local cmd alias_cmd
  cmd="${FZF_ALT_C_COMMAND:-"command alias 2> /dev/null" | sed -e 's/^alias //g' | tr -d \''"\' | cut -d= -f 1 }"
  alias_cmd=$(eval "$cmd" | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS" $(__fzfcmd) --preview "grep -w {} ~/.dotfiles/.custom_aliases | sed -e 's/^alias //g' | cut -d= -f 2" +m --preview-window=top:10%) && printf '%q' "$alias_cmd"
}
