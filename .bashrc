...
# Silence the macOS bash deprecation notice (we're still on bash, not zsh)
...
export PATH=/opt/homebrew/bin:$HOME/bin:$HOME/.dotfiles/bin:$HOME/go/bin:/usr/local/sbin:/usr/local/go/bin:$HOME/.local/bin:/usr/local/opt/llvm/bin:$HOME/.yarn/bin:$HOME/.cargo/bin:$HOME/.tfenv/bin:/usr/local/opt/libpq/bin:$HOME/.local/go/bin:$PATH
...
# Detect what to do with child nvim in floaterm aka detect parent ($NVIM is set by nvim's :terminal).
if [ -n "$NVIM" ]; then
